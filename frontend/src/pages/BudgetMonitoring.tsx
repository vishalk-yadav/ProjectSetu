    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-black text-[#0F223D] font-heading tracking-tight">
          Portfolio Budget & Fiscal Monitoring
        </h1>
        <p className="text-xs text-slate-500 mt-1">
          Macro financial surveillance, burn rate telemetry, and 80% utilization breach safeguards.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-2xl bg-white border border-slate-200/80 shadow-2xs">
          <p className="text-[10px] font-bold uppercase text-slate-500">Total Sanctioned Outlay</p>
          <p className="text-2xl font-black text-[#0F223D] font-heading mt-1">
            {formatCurrencyINR(overview.totalAllocatedBudget)}
          </p>
          <span className="text-xs text-slate-500">Approved across all active mandates</span>
        </div>

        <div className="p-5 rounded-2xl bg-white border border-slate-200/80 shadow-2xs">
          <p className="text-[10px] font-bold uppercase text-slate-500">Total Outlay Disbursed</p>
          <p className="text-2xl font-black text-indigo-600 font-heading mt-1">
            {formatCurrencyINR(overview.totalBudgetUtilized)}
          </p>
          <span className="text-xs text-slate-500">{overview.budgetUtilizationPercentage}% portfolio burn rate</span>
        </div>

        <div className="p-5 rounded-2xl bg-white border border-slate-200/80 shadow-2xs">
          <p className="text-[10px] font-bold uppercase text-slate-500">Remaining Unspent Balance</p>
          <p className="text-2xl font-black text-emerald-600 font-heading mt-1">
            {formatCurrencyINR(overview.remainingBudget)}
          </p>
          <span className="text-xs text-slate-500">Available unencumbered balance</span>
        </div>

        <div className="p-5 rounded-2xl bg-white border border-slate-200/80 shadow-2xs">
          <p className="text-[10px] font-bold uppercase text-slate-500">80% Threshold Warnings</p>
          <div className="flex items-baseline gap-2 mt-1">
            <p className="text-2xl font-black text-rose-600 font-heading">
              {overview.warningProjectsCount} Projects
            </p>
          </div>
          <span className="text-xs text-rose-600 font-medium">Requires expenditure scrutiny</span>
        </div>
      </div>
        ...
        <div className="bg-rose-50/70 border border-rose-200 rounded-2xl p-5 shadow-2xs space-y-3">
          <div className="flex items-center gap-2 text-rose-700">
            ...
            <h3 className="text-xs font-bold font-heading uppercase tracking-wider">
              Automatic Fiscal Warning: Budget Utilization Exceeds 80% Threshold
            </h3>
          </div>
          <p className="text-xs text-slate-700">
            The following projects have consumed more than 80% of sanctioned public funds. Physical verification and cost-to-complete audits are strongly advised:
          </p>
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-2xs space-y-4">
        <div>
          <h3 className="text-sm font-bold text-[#0F223D] font-heading">
            Ministry-Wise Sanctioned vs Disbursed Outlays (₹ Crore)
          </h3>
          <p className="text-xs text-slate-500">Comparative financial absorption across Union Ministries.</p>
        </div>
        ...
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData}>
              ...
              <Bar dataKey="Allocated" fill="#1A73E8" radius={[4, 4, 0, 0]} name="Sanctioned Outlay" />
              <Bar dataKey="Utilized" fill="#8b5cf6" radius={[4, 4, 0, 0]} name="Utilized Outlay" />
              <Bar dataKey="Remaining" fill="#10b981" radius={[4, 4, 0, 0]} name="Remaining Balance" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-2xs space-y-4">
        <h3 className="text-sm font-bold text-[#0F223D] font-heading">
          Recent Public Fund Disbursements
        </h3>
